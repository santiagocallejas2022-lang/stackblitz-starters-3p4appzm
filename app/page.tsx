    ...
    try {
      ...
      setPedidosOnline(
        pedidosData.map((pedido: any) => ({
          ...
          repartidor:
            pedido.empresa_envio || pedido.repartidor || "",
          codigoSeguimiento: pedido.codigo_seguimiento || "",
          enlaceSeguimiento: pedido.enlace_seguimiento || "",
          estadoGestion:
            pedido.estado_gestion || pedido.estado || "nuevo",
          sincronizacionEstado:
            pedido.sincronizacion_estado || "sincronizado",
          sincronizacionError:
            pedido.sincronizacion_error || "",
...
function Productos({
  ...
  const [herramientaProductos, setHerramientaProductos] = useState<
    "importar" | "precios" | null
  >(null);
  const [archivoImportacionNombre, setArchivoImportacionNombre] = useState("");
  const [origenSistema, setOrigenSistema] = useState("Excel / otro sistema");
  const [modoImportacion, setModoImportacion] = useState<
    "crear_actualizar" | "solo_crear" | "solo_actualizar"
  >("crear_actualizar");
  const [columnasImportacion, setColumnasImportacion] = useState<string[]>([]);
  ...
  const [tipoCambioPrecio, setTipoCambioPrecio] = useState<
    "porcentaje" | "monto" | "fijar"
  >("porcentaje");
  const [valorCambioPrecio, setValorCambioPrecio] = useState("");
  const [alcancePrecios, setAlcancePrecios] = useState<
    "todos" | "categoria" | "seleccion"
  >("todos");
  const [categoriaPrecios, setCategoriaPrecios] = useState("");
  const [productosPrecioSeleccionados, setProductosPrecioSeleccionados] =
    useState<number[]>([]);
  const [redondeoPrecios, setRedondeoPrecios] = useState("0");
  const [previsualizacionPrecios, setPrevisualizacionPrecios] = useState<any[]>(
    [],
  );
  ...
  const CAMPOS_IMPORTACION = [
    ...
    { clave: "codigo", etiqueta: "Código / SKU", obligatorio: false },
    { clave: "categoria", etiqueta: "Categoría", obligatorio: false },
    { clave: "precio", etiqueta: "Precio de venta", obligatorio: false },
    { clave: "costo", etiqueta: "Costo", obligatorio: false },
    { clave: "stock", etiqueta: "Stock", obligatorio: false },
    { clave: "minimo", etiqueta: "Stock mínimo", obligatorio: false },
    { clave: "activo", etiqueta: "Activo", obligatorio: false },
  ] as const;
  ...
  function buscarColumnaAutomatica(
    columnas: string[],
    aliases: string[],
  ) {
    const aliasesNormalizados = aliases.map(normalizarEncabezado);

    return (
      columnas.find((columna) => {
        const normalizada = normalizarEncabezado(columna);
        return aliasesNormalizados.includes(normalizada);
      }) ||
      columnas.find((columna) => {
        const normalizada = normalizarEncabezado(columna);
        return aliasesNormalizados.some(
          (alias) =>
            normalizada.includes(alias) ||
            alias.includes(normalizada),
        );
      }) ||
      ""
    );
  ...
  function crearMapeoAutomatico(columnas: string[]) {
    return {
      nombre: buscarColumnaAutomatica(columnas, [
        "nombre",
        "producto",
        "descripcion",
        "articulo",
      ...
      codigo: buscarColumnaAutomatica(columnas, [
        "codigo",
        "codigo de barras",
        "sku",
        "barcode",
      ...
      categoria: buscarColumnaAutomatica(columnas, [
        "categoria",
        "rubro",
        "familia",
        "grupo",
      ]),
      precio: buscarColumnaAutomatica(columnas, [
        "precio",
        "precio venta",
        "precio de venta",
        "p venta",
      ...
      costo: buscarColumnaAutomatica(columnas, [
        "costo",
        "coste",
        "precio costo",
        "precio de costo",
      ...
      stock: buscarColumnaAutomatica(columnas, [
        "stock",
        "existencia",
        "existencias",
        "cantidad",
      ...
      minimo: buscarColumnaAutomatica(columnas, [
        "minimo",
        "stock minimo",
        "punto reposicion",
        "alerta stock",
      ]),
      activo: buscarColumnaAutomatica(columnas, [
        "activo",
        "estado",
        "habilitado",
      ]),
  ...
  function valorFila(
    ...
    const columna = mapeoImportacion[campo];
    if (!columna) return "";
  ...
  function buscarProductoExistente(
    ...
    if (!nombreNormalizado) return undefined;
  ...
  function construirFilasNormalizadas() {
    return filasImportacion
      .map((fila) => {
        const nombreArchivo = valorFila(fila, "nombre");
        const codigoArchivo = valorFila(fila, "codigo");

        const existente = buscarProductoExistente(
          nombreArchivo,
          codigoArchivo,
        );

        const nombre =
          nombreArchivo || existente?.nombre || "";
        const codigo =
          codigoArchivo || existente?.codigo || "";

        return {
          nombre,
          codigo,
          categoria:
            valorFila(fila, "categoria") ||
            existente?.categoria ||
            "Sin categoría",
          precio:
            valorFila(fila, "precio") ||
            String(existente?.precio ?? 0),
          costo:
            valorFila(fila, "costo") ||
            String(existente?.costo ?? 0),
          stock:
            valorFila(fila, "stock") ||
            String(existente?.stock ?? 0),
          minimo:
            valorFila(fila, "minimo") ||
            String(existente?.minimo ?? 0),
          activo:
            valorFila(fila, "activo") ||
            String(existente?.activo ?? true),
        };
      })
      .filter((fila) => fila.nombre.trim());
  }

  async function leerArchivoImportacion(
    ...
    try {
      ...
      const columnas = Array.from(
        new Set(
          filas.flatMap((fila) => Object.keys(fila)),
        ),
      );

      setArchivoImportacionNombre(archivoSeleccionado.name);
      setFilasImportacion(filas);
      setColumnasImportacion(columnas);
      setMapeoImportacion(crearMapeoAutomatico(columnas));
    } catch (error) {
  ...
  async function descargarPlantillaProductos() {
    ...
    const hoja = XLSX.utils.json_to_sheet(filasEjemplo);
  ...
  async function confirmarImportacionProductos() {
    ...
    if (!mapeoImportacion.nombre) {
      alert(
        "Elegí qué columna contiene el nombre del producto.",
      );
      return;
    }

    const filas = construirFilasNormalizadas();
    ...
    const mensaje =
      `Se procesarán ${filas.length} productos. ` +
      "Los cambios se aplicarán al confirmar. ¿Continuar?";
    ...
    const { data, error } = await supabase.rpc(
      "importar_productos_masivo",
      {
        ...
        p_filas: filas,
        p_nombre_archivo:
          archivoImportacionNombre || null,
        p_origen_sistema:
          origenSistema.trim() || "Archivo",
    ...
    limpiarImportacion();
    setHerramientaProductos(null);
  }

  function idsParaActualizacionPrecios() {
    return alcancePrecios === "seleccion"
      ? productosPrecioSeleccionados
      : null;
  }

  function categoriaParaActualizacionPrecios() {
    return alcancePrecios === "categoria"
      ? categoriaPrecios.trim() || null
      : null;
  }

  async function previsualizarActualizacionPrecios() {
    if (!comercioActual) {
      alert("No hay comercio asociado.");
      return;
    }

    const valor = Number(valorCambioPrecio);
    ...
    if (
      alcancePrecios === "categoria" &&
      !categoriaPrecios.trim()
    ) {
    ...
    const { data, error } = await supabase.rpc(
      "previsualizar_actualizacion_precios",
      {
        p_comercio_id: comercioActual.id,
        p_tipo: tipoCambioPrecio,
        p_valor: valor,
        p_categoria:
          categoriaParaActualizacionPrecios(),
        p_producto_ids: idsParaActualizacionPrecios(),
        p_redondeo: Number(redondeoPrecios) || 0,
        p_solo_activos: true,
      },
    );
    ...
    setPrevisualizacionPrecios(data || []);
  }

  async function aplicarActualizacionPrecios() {
    if (!comercioActual) return;

    if (previsualizacionPrecios.length === 0) {
      alert("Primero generá la vista previa.");
      return;
    }

    if (
      !confirm(
        `Se modificarán ${previsualizacionPrecios.length} precios. ¿Confirmás el cambio?`,
      )
    ) {
      return;
    }
    ...
    const { data, error } = await supabase.rpc(
      "actualizar_precios_masivo",
      {
        p_comercio_id: comercioActual.id,
        p_tipo: tipoCambioPrecio,
        p_valor: Number(valorCambioPrecio),
        p_categoria:
          categoriaParaActualizacionPrecios(),
        p_producto_ids: idsParaActualizacionPrecios(),
        p_redondeo: Number(redondeoPrecios) || 0,
    ...
    if (error) {
      alert(
        "No se pudieron modificar los precios: " +
          error.message,
      );
      return;
    }
    ...
    await recargarDatos();
    setPrevisualizacionPrecios([]);

    alert(
      `${data?.productos_actualizados ?? 0} precios actualizados correctamente.`,
    );
  }

  function alternarProductoPrecio(productoId: number) {
    setProductosPrecioSeleccionados((actuales) =>
  ...
  return (
    <>
      <Header
        title="Productos"
        subtitle="Alta, edición, migración, precios y control de stock."
        action={
          <div
            style={{
              display: "flex",
      ...
      {herramientaProductos === "importar" && (
        <Panel title="Importar o migrar productos">
          <p style={{ ...styles.text, marginTop: 0 }}>
            Subí un Excel, CSV o archivo exportado por otro
            sistema. Antes de confirmar vas a poder relacionar
            sus columnas con las del sistema.
          </p>
          ...
          <div className="app-form-grid" style={styles.formGrid}>
            <div>
              ...
              <input
                ...
                onChange={leerArchivoImportacion}
                style={{
                  width: "100%",
                  padding: 10,
                  border: "1px solid #cbd5e1",
              ...
              {archivoImportacionNombre && (
                <small style={{ color: "#475569" }}>
                  {archivoImportacionNombre} ·{" "}
                  {filasImportacion.length} filas
                </small>
              )}
            </div>

            <Input
              placeholder="Sistema de origen (opcional)"
              value={origenSistema}
              onChange={setOrigenSistema}
            />
            ...
            <div>
              ...
              >
                <option value="crear_actualizar">
                  Crear nuevos y actualizar existentes
                </option>
                <option value="solo_crear">
                  Crear nuevos y omitir existentes
                </option>
                <option value="solo_actualizar">
                  Actualizar existentes solamente
                </option>
              </select>
            </div>
          </div>
          ...
          <div className="app-actions" style={styles.actions}>
            <SecondaryButton onClick={descargarPlantillaProductos}>
              Descargar plantilla
            </SecondaryButton>
            <SecondaryButton
              onClick={() => {
          ...
          {columnasImportacion.length > 0 && (
            <>
              <h3 style={styles.panelTitle}>Relacionar columnas</h3>
              <div className="app-form-grid" style={styles.formGrid}>
                {CAMPOS_IMPORTACION.map((campo) => (
                  <div key={campo.clave}>
                    ...
                    <select
                      value={mapeoImportacion[campo.clave] || ""}
                      onChange={(event) =>
                        setMapeoImportacion({
                          ...mapeoImportacion,
                    ...
                    >
                      ...
                      {columnasImportacion.map((columna) => (
              ...
              <h3 style={styles.panelTitle}>Vista previa</h3>
              <Table>
                <thead style={styles.thead}>
                  <tr>
                    <Th>Nombre</Th>
                ...
                <tbody>
                  {construirFilasNormalizadas()
                    .slice(0, 8)
                    .map((fila, indice) => (
                      <tr key={`${fila.codigo}-${indice}`} style={styles.tr}>
                        <Td>{fila.nombre}</Td>
                        <Td>{fila.codigo || "Se generará"}</Td>
              ...
              <div className="app-actions" style={styles.actions}>
                <Button onClick={confirmarImportacionProductos}>
                  {procesandoImportacion
                    ? "Importando..."
                    : `Importar ${construirFilasNormalizadas().length} productos`}
                </Button>
              </div>
            </>
          )}
      ...
      {herramientaProductos === "precios" && (
        <Panel title="Modificar precios en conjunto">
          <p style={{ ...styles.text, marginTop: 0 }}>
            Podés aumentar, disminuir o fijar precios para
            todo el comercio, una categoría o productos
            seleccionados. Usá valores negativos para bajar
            precios.
          </p>
          ...
          <div className="app-form-grid" style={styles.formGrid}>
            <div>
              ...
              >
                ...
                <option value="fijar">Fijar el mismo precio</option>
              </select>
            </div>

            <Input
              placeholder={
                tipoCambioPrecio === "porcentaje"
                  ? "Ej.: 15 o -10"
                  : "Valor"
              }
              type="number"
              value={valorCambioPrecio}
              onChange={(valor) => {
                setValorCambioPrecio(valor);
                setPrevisualizacionPrecios([]);
              }}
            />
            ...
            {alcancePrecios === "categoria" && (
              <div>
                ...
                <select
                  value={categoriaPrecios}
                  onChange={(event) => {
                    setCategoriaPrecios(event.target.value);
                    setPrevisualizacionPrecios([]);
                  }}
                  style={{
                    width: "100%",
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  setRedondeoPrecios(event.target.value);
                  setPrevisualizacionPrecios([]);
                }}
                style={{
                  width: "100%",
              ...
              >
                ...
                <option value="10">Al múltiplo de $10</option>
                <option value="100">Al múltiplo de $100</option>
                <option value="500">Al múltiplo de $500</option>
                <option value="1000">Al múltiplo de $1.000</option>
              </select>
            </div>
          </div>
              ...
              {productos
                ...
                .map((producto) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={productosPrecioSeleccionados.includes(
                        producto.id,
                      )}
                      onChange={() => alternarProductoPrecio(producto.id)}
                    />
          ...
          <div className="app-actions" style={styles.actions}>
            <Button onClick={previsualizarActualizacionPrecios}>
              {procesandoPrecios ? "Calculando..." : "Ver precios nuevos"}
            </Button>
            <SecondaryButton
              onClick={() => {
                setHerramientaProductos(null);
          ...
          {previsualizacionPrecios.length > 0 && (
            <>
              <h3 style={styles.panelTitle}>
                Vista previa de {previsualizacionPrecios.length} productos
              </h3>
              ...
              <Table>
                ...
                <tbody>
                  {previsualizacionPrecios.slice(0, 50).map((fila) => (
                    <tr key={fila.producto_id} style={styles.tr}>
                      <Td>{fila.nombre_producto}</Td>
                      <Td>{fila.categoria_producto}</Td>
                      <Td>{money(Number(fila.precio_actual))}</Td>
                      <Td>
                        <strong>{money(Number(fila.precio_nuevo))}</strong>
                      </Td>
                    </tr>
                  ))}
                </tbody>
              ...
              {previsualizacionPrecios.length > 50 && (
                <p style={styles.text}>
                  Se muestran los primeros 50 productos.
                </p>
              )}

              <div className="app-actions" style={styles.actions}>
                <Button onClick={aplicarActualizacionPrecios}>
                  {procesandoPrecios ? "Aplicando..." : "Confirmar cambios"}
                </Button>
              </div>
            </>
          )}
...
function PedidosOnline({
  ...
  function tieneProblema(pedido: PedidoOnline) {
    return (
      pedido.sincronizacionEstado === "error" ||
  ...
  function descripcionSiguienteAccion(pedido: PedidoOnline) {
    if (pedido.sincronizacionEstado === "sincronizando") {
      return "Sincronizando con Tiendanube...";
    }
    ...
    if (pedido.estado === "preparacion") {
      return pedido.tipoEntrega === "retiro"
        ? "Listo para retirar"
        : "Marcar como listo";
    }
  ...
  function esPedidoRealDeTiendanube(pedido: PedidoOnline) {
    return (
      pedido.canal.trim().toLowerCase() === "tiendanube" &&
      Boolean(pedido.pedidoExternoId)
    );
  }

  async function invocarAccionTiendanube(
    pedido: PedidoOnline,
    accion:
      | "preparacion"
      | "listo"
    ...
    if (error) {
      throw new Error(error.message || "Falló la comunicación con Tiendanube.");
    }

    if (!data?.ok) {
      throw new Error(data?.error || "Tiendanube rechazó la acción.");
    }

    return data;
  }

  async function avanzarPedidoLocal(pedido: PedidoOnline) {
  ...
  async function avanzarPedido(pedido: PedidoOnline) {
    ...
    if (pedido.sincronizacionEstado === "sincronizando") {
      alert("El pedido ya se está sincronizando.");
      return;
    }

    try {
      if (!esPedidoRealDeTiendanube(pedido)) {
        await avanzarPedidoLocal(pedido);
        await recargarDatos();
        return;
      }
      ...
      if (pedido.estado === "nuevo") {
        ...
        const empresaEnvio = prompt(
          "Empresa de envío",
          pedido.repartidor || "",
        );

        if (empresaEnvio === null) return;

        if (empresaEnvio.trim().length < 2) {
          alert("Ingresá la empresa de envío.");
          return;
        }

        const codigoSeguimiento = prompt(
        ...
        if (codigoSeguimiento.trim().length < 2) {
          alert("Ingresá el código de seguimiento.");
          return;
        }

        const enlaceSeguimiento = prompt(
          "Enlace de seguimiento opcional",
          pedido.enlaceSeguimiento || "",
        );

        if (enlaceSeguimiento === null) return;

        const notificarCliente = confirm(
          "¿Querés que Tiendanube avise al cliente sobre el envío?",
        );

        accion = "enviado";
        payload = {
          empresa_envio: empresaEnvio.trim(),
          codigo_seguimiento: codigoSeguimiento.trim(),
          enlace_seguimiento:
            enlaceSeguimiento.trim() || null,
        ...
        if (!confirm("¿Confirmás que el cliente retiró el pedido?")) {
          return;
        }

        accion = "entregado";
      } else if (pedido.estado === "enviado") {
        if (!confirm("¿Confirmás que el pedido fue entregado?")) {
          return;
        }
      ...
      await invocarAccionTiendanube(pedido, accion, payload);
      await recargarDatos();
    } catch (error) {
      console.error("Error al gestionar pedido:", error);
      alert(
  ...
  async function cancelarPedido(pedido: PedidoOnline) {
    ...
    const codigoMotivo = motivos[opcion.trim()] || "other";

    if (
      !confirm(
        "Se cancelará también en Tiendanube, se repondrá el stock y se avisará al cliente. El reembolso del dinero se controla por separado. ¿Continuar?",
      )
    ) {
      return;
    }
    ...
    try {
      await invocarAccionTiendanube(
        pedido,
        "cancelar",
        {
          motivo_cancelacion: motivo.trim(),
          codigo_motivo_cancelacion: codigoMotivo,
          notificar_cliente: true,
          reponer_stock_tiendanube: true,
        },
      );
      ...
      alert(
        "Pedido cancelado en el sistema y en Tiendanube. Revisá el reembolso en el medio de pago.",
      );
    } catch (error) {
        ...
        <Panel title="Próxima tarea recomendada">
          ...
          >
            ...
            {proximaTarea.estadoPago !== "aprobado" && puedeGestionar ? (
              ...
              <Button
                onClick={() => avanzarPedido(proximaTarea)}
                disabled={
                  proximaTarea.sincronizacionEstado ===
                  "sincronizando"
                }
              >
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      ...
                      >
                        {pedido.estadoGestion === "listo_retiro"
                          ? "Listo para retirar"
                          : estado.etiqueta}
                      </span>
                      <span
                ...
                {(pedido.repartidor ||
                  ...
                  <p style={{ margin: "12px 0 0", color: "#475569" }}>
                    Entrega: {pedido.repartidor || "Sin empresa"}
                    {pedido.codigoSeguimiento
                ...
                {pedido.sincronizacionEstado === "sincronizando" && (
                  <p
                    style={{
                      margin: "10px 0 0",
                      color: "#1d4ed8",
                ...
                >
                  ...
                  {pedido.sincronizacionEstado === "error" &&
                    esPedidoRealDeTiendanube(pedido) &&
                    puedeGestionar && (
                      <button
                        type="button"
                        style={{
                        ...
                        onClick={() =>
                          reintentarSincronizacion(pedido)
                        }
                      >
